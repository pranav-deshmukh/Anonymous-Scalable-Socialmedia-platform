...
export const signup = async (
  ...
  try {
    ...
    const verificationotp = jwt.sign({userId:newUser._id},process.env.JWT_SECRET!,{expiresIn:'15m'});

    

    const verificationUrl = `${req.protocol}://${req.get('host')}/api/v1/users/verify-email/${verificationotp}`;
